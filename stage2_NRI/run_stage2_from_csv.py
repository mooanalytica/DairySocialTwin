...
DEFAULT_VALENCE_CKPT = BASE / "models_new" / "stage2_valence_nri_best.pt"
...
VALENCE_BIDIRECTIONAL_MODE = "mean"
...
def merge_valence_direction_logits(logits_list: list[np.ndarray], mode: str = VALENCE_BIDIRECTIONAL_MODE) -> np.ndarray:
    if not logits_list:
        return np.zeros((0,), dtype=np.float32)
    mode = str(mode or "off").lower()
    if mode == "off" or len(logits_list) == 1:
        return np.asarray(logits_list[0], dtype=np.float32)
    if mode == "confidence":
        return np.asarray(max(logits_list, key=lambda item: float(np.max(_softmax_np(item)))), dtype=np.float32)
    if mode == "mean":
        probs = np.mean(np.stack([_softmax_np(item) for item in logits_list], axis=0), axis=0)
...
def load_models(args, device: torch.device) -> LoadedModels:
    ...
    if not required_kps:
        raise GlobalStage2Error("checkpoints do not contain keypoints")

    valence_schema = str(valence_ckpt.get("feature_schema", "legacy_absolute_v0"))
    if (
        str(valence_ckpt.get("model", "")) != "NRIValenceHead"
        or valence_schema != NRI_VALENCE_FEATURE_SCHEMA_VERSION
    ):
        raise GlobalStage2Error(
            f"Valence checkpoint must be NRIValenceHead/{NRI_VALENCE_FEATURE_SCHEMA_VERSION}; "
            f"got model={valence_ckpt.get('model', '')!r} feature_schema={valence_schema!r}. "
            "Retrain the valence classifier with train_stage2_nri.py --task valence_2class."
        )

    interaction_gate_schema = str(interaction_gate_ckpt.get("feature_schema", "legacy_absolute_v0"))
    if (
        str(interaction_gate_ckpt.get("model", "")) != "NRIInteractionHead"
        or interaction_gate_schema != NRI_FEATURE_SCHEMA_VERSION
    ):
        raise GlobalStage2Error(
            f"Interaction gate checkpoint must be NRIInteractionHead/{NRI_FEATURE_SCHEMA_VERSION}; "
            f"got model={interaction_gate_ckpt.get('model', '')!r} feature_schema={interaction_gate_schema!r}. "
            "Retrain the gate with train_stage2_nri.py --task interaction_gate_2class."
        )
    interaction_gate_model_type = "nri"
    ...
    nri_cfg = dict(interaction_gate_ckpt.get("nri", {}))
    if not nri_cfg:
        raise GlobalStage2Error("NRI interaction gate checkpoint missing 'nri' config.")
    if int(nri_cfg.get("num_keypoints", -1)) != len(required_kps):
        raise GlobalStage2Error(
            f"NRI gate num_keypoints={nri_cfg.get('num_keypoints')}; expected {len(required_kps)} from valence keypoints."
        )
    interaction_gate_model = NRIInteractionHead(**nri_cfg).to(device)
    interaction_gate_max_frames = int(interaction_gate_ckpt.get("max_frames", 64))
    ...
    valence_cfg = dict(valence_ckpt.get("nri_valence", {}))
    if not valence_cfg:
        raise GlobalStage2Error("NRI valence checkpoint missing 'nri_valence' config.")
    if int(valence_cfg.get("num_keypoints", -1)) != len(required_kps):
        raise GlobalStage2Error(
            f"NRI valence num_keypoints={valence_cfg.get('num_keypoints')}; expected {len(required_kps)} from checkpoint keypoints."
        )
    valence_model = NRIValenceHead(**valence_cfg).to(device)
    valence_max_frames = int(valence_ckpt.get("max_frames", 64))
    ...
    gate_shape = f"nri_gate_joints={len(required_kps)} max_frames={interaction_gate_max_frames}"
    valence_shape = f"nri_valence_joints={len(required_kps)} max_frames={valence_max_frames}"
    log(f"[info] keypoints={len(required_kps)} {gate_shape} {valence_shape}")
...
def gate_seq_from_pairbuf(stbuf: dict, num_kpts: int) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    for t in range(t_len):
        keypoints[t, 0], person_mask[t, 0] = _clean_gate_person(k_a_list[t], num_kpts)
        keypoints[t, 1], person_mask[t, 1] = _clean_gate_person(k_b_list[t], num_kpts)
...
def nri_gate_batch_probs(
    ...
    for start in range(0, len(list_items), batch_size):
        ...
        person_frame_mask = torch.from_numpy(person_mask).to(device=device, dtype=torch.bool)
        m_t = person_frame_mask.any(dim=1)
        frame_mask = torch.zeros(len(chunk), max_len, dtype=torch.bool, device=device)
        for i, (seq, _mask) in enumerate(chunk):
            frame_mask[i, : seq.shape[0]] = True
        with torch.cuda.amp.autocast(enabled=(amp and device.type == "cuda"), dtype=torch.float16):
            probs = model(k_t, m_t, frame_mask=frame_mask)["p_video"]
...
def valence_batch_logits(
    ...
    for start in range(0, len(list_items), batch_size):
        ...
        for i, (seq, _mask) in enumerate(chunk):
            frame_mask[i, : seq.shape[0]] = True
        with torch.cuda.amp.autocast(enabled=(amp and device.type == "cuda"), dtype=torch.float16):
            logits = model(k_t, m_t, frame_mask=frame_mask)["logits"]
    ...
    for frame_idx in pbar:
        ...
        for a_id, b_id in suspect_pairs:
            ...
            if (frame_idx % classify_every) == 0:
                ...
                interaction_gate_seq_map: dict[tuple[int, int], tuple[np.ndarray, np.ndarray]] = {}
                valence_seq_map: dict[tuple[int, int], tuple[np.ndarray, np.ndarray]] = {}
                for key in suspect_pairs:
                    ...
                    if len(stbuf["frames"]) < int(min(Q_WINDOWS_SEC) * fps):
                        continue
                    interaction_gate_seq = gate_seq_from_pairbuf(stbuf, num_kpts)
                    if interaction_gate_seq is None:
                        continue
                    if interaction_gate_seq[0].shape[0] < 2:
                        continue
                    eligible_keys.append(key)
                    interaction_gate_seq_map[key] = interaction_gate_seq
                    valence_seq_map[key] = interaction_gate_seq

            if eligible_keys:
                p1_max = {k: 0.0 for k in eligible_keys}
                for w in MI_CROPS_SEC:
                    keys_w: list[tuple[int, int]] = []
                    list_gate: list[tuple[np.ndarray, np.ndarray]] = []
                    for key in eligible_keys:
                        kfull, mfull = interaction_gate_seq_map[key]
                        kseq, mseq = crop_gate_sequence(kfull, mfull, fps, w, models.interaction_gate_max_frames)
                        list_gate.append((kseq, mseq))
                        keys_w.append(key)
                        if BIDIRECTIONAL_PAIR_INFERENCE:
                            list_gate.append(swap_gate_pair_sequence(kseq, mseq))
                            keys_w.append(key)
                    probs = nri_gate_batch_probs(
                ...
                if stable_interaction_gate_keys:
                    ...
                    for w in MI_CROPS_SEC:
                        list_valence: list[tuple[np.ndarray, np.ndarray]] = []
                        keys_w: list[tuple[tuple[int, int], bool]] = []
                        for key in stable_interaction_gate_keys:
                            kfull, mfull = valence_seq_map[key]
                            kseq, mseq = crop_gate_sequence(kfull, mfull, fps, w, models.valence_max_frames)
                            list_valence.append((kseq, mseq))
                            keys_w.append((key, False))
                            if BIDIRECTIONAL_PAIR_INFERENCE and VALENCE_BIDIRECTIONAL_MODE != "off":
                                list_valence.append(swap_gate_pair_sequence(kseq, mseq))
                        ...
                        direction_logits: dict[tuple[int, int], list[np.ndarray]] = defaultdict(list)
                        for (key, _swapped), logits in zip(keys_w, logits_batch):
                            direction_logits[key].append(logits.astype(np.float32))

                        for key, logits_items in direction_logits.items():
                            logits = merge_valence_direction_logits(logits_items)
...
def build_manifest(
    ...
    feature_csv_manifest = {
        ...
        "interaction_gate_feature_schema": NRI_FEATURE_SCHEMA_VERSION,
        "valence_feature_schema": NRI_VALENCE_FEATURE_SCHEMA_VERSION,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--input-root", default=str(DEFAULT_INPUT_ROOT), help="Root containing Stage-1 tracking_boxes.csv and keypoints.csv outputs.")
