...
def load_models(args, device: torch.device) -> LoadedModels:
    ...
    if str(valence_ckpt.get("model", "")) != "ValenceTransformer":
        raise GlobalStage2Error(
            ...
            "Retrain the valence classifier with train_stage2_transformer.py --task valence_2class."
        )
    if valence_schema != FEATURE_SCHEMA_VERSION:
        raise GlobalStage2Error(
            ...
            "Retrain the valence classifier with train_stage2_transformer.py."
        )
    ...
    if (
        ...
        raise GlobalStage2Error(
            ...
            "Retrain the gate with train_stage2_transformer.py --task interaction_gate_2class."
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--input-root", default=str(DEFAULT_INPUT_ROOT), help="Root containing precomputed tracking_boxes.csv and keypoints.csv files.")
