"""Strict configuration for deterministic S04 conflict-group review selection."""
...
def load_s04_conflict_review_config(
    path: Path,
) -> tuple[S04ConflictReviewConfig, dict[str, Any], str]:
    """Load the closed conflict-review selection config and canonical hash."""

    path = path.resolve()
    if not path.is_file():
        raise ContractError(f"S04 conflict review config does not exist: {path}")
    try:
        payload = yaml.safe_load(path.read_bytes())
    except (OSError, yaml.YAMLError) as exc:
        raise ContractError(
            f"cannot read S04 conflict review config {path}: {exc}"
        ) from exc
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise ContractError(
            "S04 conflict review config must contain exactly "
    ...
    if not all(
        ...
        raise ContractError("S04 conflict review config sections must be mappings")
    expected = {
    ...
    result = S04ConflictReviewConfig(
        schema_version=str(_required(pipeline, "schema_version", "pipeline")),
        random_seed=_integer(pipeline, "random_seed", "pipeline"),
        largest_group_count=_integer(
            selection, "largest_group_count", "selection"
        ),
        random_group_count=_integer(selection, "random_group_count", "selection"),
        max_edges_per_group=_integer(
            selection, "max_edges_per_group", "selection"
        ),
    ...
    if result.schema_version != "1.0":
        raise ContractError(
            "fixed S04 conflict review requires pipeline.schema_version=1.0"
        )
    if result.random_seed != 20260710:
        raise ContractError(
            "fixed S04 conflict review requires pipeline.random_seed=20260710"
        )
    if (
        result.largest_group_count,
        result.random_group_count,
        result.max_edges_per_group,
    ) != (3, 3, 3):
        raise ContractError(
            "fixed S04 conflict review selection must be exactly "
            "largest_groups=3, random_groups=3, max_edges_per_group=3"
        )
    if result.source_tail_sec != 2.0 or result.target_head_sec != 2.0:
        raise ContractError(
            "fixed S04 conflict review windows must be exactly 2 seconds per side"
        )
    if (result.output_width, result.output_height) != (1920, 1080):
        raise ContractError(
            "fixed S04 conflict review output must be exactly 1920x1080"
        )
    ...
    if str(_required(encoder, "codec", "encoder")) != "h264_nvenc":
        raise ContractError("S04 conflict review requires encoder.codec=h264_nvenc")
    if result.logical_gpu != 0:
        raise ContractError(
            "with CUDA_VISIBLE_DEVICES=1, S04 conflict review requires "
            "encoder.logical_gpu=0"
        )
    ...
    if result.pixel_format != "yuv420p":
        raise ContractError(
            "S04 conflict review requires encoder.pixel_format=yuv420p"
        )
...
__all__ = ["S04ConflictReviewConfig", "load_s04_conflict_review_config"]
