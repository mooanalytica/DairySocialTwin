"""Single-frame bbox-only raster annotation for S05 evidence review.

Boxes are interpreted in the original raw-frame coordinate system.  The
whole annotated frame may then be resized without changing individual box
geometry.  No autorotation, labels, scores, legends, keypoints, or synthetic
geometry are available through this API.
"""
...
def render_s05_review_frame(
    ...
    *,
    ...
    """Draw four bbox roles on a copy of one raw, unrotated landscape frame."""
    ...
    if output_width * raw_height != output_height * raw_width:
        raise ValueError("output dimensions must preserve raw landscape aspect ratio")

    roles = (
        ("unrelated_boxes", unrelated_boxes, UNRELATED_BGR, _UNRELATED_THICKNESS),
        (
            "intermediate_boxes",
            intermediate_boxes,
            INTERMEDIATE_BGR,
...
def _validate_frame(frame: RawFrame) -> tuple[int, int]:
    ...
    if width <= height or width * 9 != height * 16:
        raise ValueError("raw_frame must use unrotated 16:9 landscape geometry")
    return width, height
...
def _as_boxes(
    ...
    try:
        ...
        raise ValueError(f"{label} must contain numeric boxes") from exc
    if array.size == 0:
